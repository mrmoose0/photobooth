...
class PhotoboothApp:
    ...
    def prepare_next_slide(self):
        if len(self.photos_list) > 1:
            # Continua a scegliere a caso finché non trova una foto diversa da quella attuale
            while True:
                next_path = random.choice(self.photos_list)
                # Confronta il percorso con l'immagine attualmente in mostra
                # (per farlo ci serve sapere il path corrente, quindi lo aggiungiamo come variabile di stato se non c'è)
    ...
    def take_picture_and_upload(self):
        ...
        if remote_path:
            ...
            self.state = "REVIEW"
            self.last_activity_time = time.time() # Resetta il timer per il timeout di inattività
        else:
            self.start_slideshow()
    ...
    def handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1: # Il tocco su schermo è letto come click sinistro
                    pos = event.pos
                    self.last_activity_time = time.time() # Resetta timeout inattività
    ...
    def update(self):
        ...
        if self.state in ["REVIEW", "QR"]:
            if now - self.last_activity_time >= self.inactivity_timeout:
                print("Timeout inattività raggiunto. Ritorno allo slideshow.")
                self.start_slideshow()
        ...
        if self.state == "SLIDESHOW":
            if not self.photos_list:
                ...
                if self.next_img and elapsed > (self.slide_duration - self.fade_duration):
                    ...
                    self.next_img.set_alpha(alpha) # Imposta l'opacità
    ...
    def draw_button(self, rect, text):
        ...
        s.fill((0, 0, 0, 150)) # Nero con opacità 150/255
